...
public class ResponseThread extends Thread {
    ...
    public ResponseThread(Worker client, PropertyChangeSupport pcs) {
        this.client = client;
        this.pcs = pcs;
    }
    ...
    private synchronized void response() {
        if (response instanceof LoginResponse login) {
            Platform.runLater(() -> {
                pcs.firePropertyChange(RequestsType.LOGIN_WORKER.toString(),null,null);
            ...
            fire(RequestsType.UPDATE_CALENDAR_WORKER.toString());
        ...
        } else if (response instanceof ConcludeServiceResponse) {
            fire(RequestsType.CONCLUDE_SERVICE.toString());
        }
    }

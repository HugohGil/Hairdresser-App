...
public class Edit {
    ...
    private void registerHandlers() {
        ...
        model.addPropertyChangeListener(RequestsType.EDIT_SERVICE.toString(), evt -> {
            model.rUpdateWeeklyCalender(hairDresser.getSelectionModel().getSelectedItem(), date);
        });
        ...
        edit.setOnMouseClicked(mouseEvent -> {
            ...
            model.rEditService(new Service(request.getId(), hairDresser.getSelectionModel().getSelectedItem().getId()
                    , date1, date2, List.of(typeOfService.getSelectionModel().getSelectedItem())), serviceId);
            selected = 0;
            isSelected = false;
        });
